  ...
  const renderContent = (content: string) => {
    ...
    return paragraphs.map((paragraph, i) => {
      ...
      if (/^\d+\. /.test(paragraph)) {
        const items = paragraph.split('\n').filter(Boolean);
        return (
          <ol key={i} className="space-y-2 my-6 list-decimal list-inside">
            {items.map((item, j) => (
              <li key={j} className="text-muted-foreground text-[17px] leading-[1.8]">
                {parseLinks(item.replace(/^\d+\. /, ''))}
              </li>
            ))}
          </ol>
        );
      ...
      <div className="flex-1">
        ...
        <article className="px-6 pt-8 pb-20">
          <div className="max-w-2xl mx-auto">
            ...
            {/* Subtitle */}
            <p className="text-muted-foreground text-lg leading-relaxed mb-12 pb-8 border-b border-border">
              {useCase.excerpt}
            </p>
